...
local on_attach = function(client, bufnr)
    ...
    vim.api.nvim_buf_set_keymap(bufnr, 'n', '<leader>ll', '<cmd>lua vim.diagnostic.open_float({ scope = "line" })<CR>',
        opts)
    vim.api.nvim_buf_set_keymap(bufnr, 'n', '<leader>lt', "",
        { noremap = opts.noremap, silent = opts.silent, callback = toggleVirtualText })

    -- Format on save
    local autoformat_filetypes = { "ruby", "lua", "typescriptreact" }
    if
